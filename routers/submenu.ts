...
router.post("/", async (req: Request, res: Response) => {
  // add pikachu to user's pokemons list
  const pikachu = await fetch("https://pokeapi.co/api/v2/pokemon/pikachu");
  const pikachuData = await pikachu.json();
  let pokemonList: any[] = [{
    pokemonId: pikachuData.id, pokemonName: pikachuData.name, pokemonLevel: 1, pokemonHP: 35, pokemonAttack: 55,
    pokemonDefense: 40, pokemonSpeed: 90, pokemonType: pikachuData.types, pokemonMoves: pikachuData.moves.splice(0, 4).map((move: any) => move.move.name),
  ...
  try {
    if (!await client.db("users").collection("usersPokemons").findOne({ _id: res.locals.user._id })) {
      await client.db("users").collection("usersPokemons").insertOne({ _id: res.locals.user._id, pokemons: pokemonList, currentPokemon: 25 });
