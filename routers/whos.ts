...
router.get("/award/:type/:points", async (req, res) => {
  const type = req.params.type;
  const points = parseInt(req.params.points);
  const currentPokemonId = parseInt(res.locals.currentPokemon.pokemonId);

  if (type === "attack") {
      if (points > 0) {
          if (await client.db("users").collection("usersPokemons").findOne({ _id: res.locals.user._id, "pokemons.pokemonId": currentPokemonId })) {
              await client.db("users").collection("usersPokemons").updateOne(
                  { _id: res.locals.user._id, "pokemons.pokemonId": currentPokemonId },
  ...
  if (type === "hp") {
      if (points > 0) {
          if (await client.db("users").collection("usersPokemons").findOne({ _id: res.locals.user._id, "pokemons.pokemonId": currentPokemonId })) {
              await client.db("users").collection("usersPokemons").updateOne(
                  { _id: res.locals.user._id, "pokemons.pokemonId": currentPokemonId },
                  { $inc: { "pokemons.$.pokemonHP": points } },
                  { upsert: true }
              );
          }
          res.status(200).json({ message: "HP is succesvol gewijzigd." });
      }
      return;
  }
});
